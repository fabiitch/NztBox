package com.nzt.box.math.quadtree.test;
...
public class QuadTT {    // ----------------------------------------------------------------------------------------
    ...
    QuadTT(int width, int height, int start_max_elements, int start_max_depth) {
...
interface IQtVisitor {
    ...
    void branch(QuadTT qt, int node, int depth, int mx, int my, int sx, int sy);

    // Called when traversing a leaf node.
    // (mx, my) indicate the center of the node's AABB.
    // (sx, sy) indicate the half-size of the node's AABB.
    void leaf(QuadTT qt, int node, int depth, int mx, int my, int sx, int sy);
}
